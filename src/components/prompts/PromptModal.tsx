...
import type { PromptEntry } from '../../data/promptsLibrary';
...
interface Props {
  ...
  /** 0 = anonymous, 1 = lead, 2 = premium subscriber. */
  tier: number;
  onClose: () => void;
}
...
function highlightTokens(text: string): ReactNode[] {
  return text.split(/(\[[^\]]+\])/g).map((part, i) =>
    /^\[[^\]]+\]$/.test(part) ? (
      <mark key={i} style={{ color: LIME, background: 'transparent' }}>{part}</mark>
    ) : (
    ...
    >
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-2 mt-4">
          ...
          {prompt.complianceTags.map((t) => <Chip key={t}>{t}</Chip>)}
        ...
        <div className="mt-6">
          {premiumLocked ? (
            <div className="text-center">
              <div className="flex justify-center mb-3" style={{ color: LIME }}><Lock size={28} /></div>
            ...
            <>
              ...
              <div className="flex flex-wrap gap-3 mt-4">
                <button
                  ...
                  title={canCopy ? 'Copy to clipboard' : 'Copying is a Premium benefit'}
                >
