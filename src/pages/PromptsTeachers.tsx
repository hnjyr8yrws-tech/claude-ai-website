  ...
  return (
    <>
      ...
      {SECTIONS.map((section) => (
        <section key={section.label} className="px-5 sm:px-8 py-10 border-t" style={{ borderColor: '#ECE7DD', background: section.label === 'Quick Wins' ? 'white' : 'var(--bg)' }}>
          <div className="max-w-3xl mx-auto">
        ...
        <div className="max-w-3xl mx-auto">
          <div className="rounded-xl border p-5" style={{ borderColor: '#ECE7DD', background: 'white' }}>
            <p className="text-sm leading-relaxed" style={{ color: '#4A4A4A' }}>
              <strong style={{ color: '#1A1A1A' }}>Important:</strong> These prompts support teacher practice but do not replace professional judgment, safeguarding duties, or school policy. Always adapt for your school context and the individual needs of students.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="px-5 sm:px-8 py-8 border-t" style={{ background: 'white', borderColor: '#ECE7DD' }}>
        <div className="max-w-3xl mx-auto flex flex-wrap gap-3">
          <Link to="/ai-training/teachers" className="text-sm font-medium px-4 py-2 rounded-lg border transition-colors hover:border-[#BEFF00] hover:text-[#BEFF00]" style={{ borderColor: '#ECE7DD', color: '#4A4A4A' }}>AI Training for Teachers →</Link>
