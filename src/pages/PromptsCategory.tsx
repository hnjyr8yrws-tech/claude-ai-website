  ...
  if (packsInCategory.length === 0) {
    return (
      <div className="px-5 sm:px-8 py-16 text-center" style={{ background: 'var(--bg)' }}>
        ...
        <Link to="/prompts/library" style={{ color: '#BEFF00' }}>← Back to library</Link>
      </div>
    );
  }
    ...
    <>
      ...
      <section className="px-5 sm:px-8 pt-12 pb-8 border-b" style={{ background: 'var(--bg)', borderColor: '#ECE7DD' }}>
        <div className="max-w-5xl mx-auto">
          ...
          {catData && (
            <p className="text-base mb-4 max-w-2xl" style={{ color: '#4A4A4A' }}>
              {catData.description}
            </p>
          )}
        ...
        <div className="max-w-5xl mx-auto">
          ...
          {filtered.length === 0 ? (
            <div className="text-center py-12">
              ...
              <button onClick={() => setSenFilter('All')} className="text-sm" style={{ color: '#BEFF00' }}>Clear filter</button>
            </div>
          ) : (
              ...
              {adjacentSlugs.map((slug) => {
                ...
                return (
                  <Link
                    ...
                    className="px-4 py-2 rounded-full border text-sm font-medium transition-colors hover:border-[#BEFF00] hover:text-[#BEFF00] focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[#BEFF00]"
