  ...
  return (
    ...
    >
      ...
      <div className="px-5 pb-5 pt-3 border-t flex items-center justify-between" style={{ borderColor: '#f3f4f6' }}>
        ...
        <a href={item.url} target="_blank" rel={linkRel} className="text-sm font-semibold px-3 py-1.5 rounded-lg" style={{ background: TEAL, color: 'var(--color-ink)' }}>
          Visit →
        </a>
      </div>
    </div>
      ...
      <section className="pt-8 pb-16 px-5 sm:px-8" style={{ background: 'var(--bg)' }}>
        <div className="max-w-4xl mx-auto">
          ...
          <h1 className="font-display text-5xl sm:text-6xl leading-[1.08] tracking-tight mb-6" style={{ color: 'var(--text)' }}>
            AI Learning for{' '}
            <span style={{ color: 'var(--color-ink-accent)' }}>Students.</span>
          </h1>
            ...
            {TOPIC_BLOCKS.map(block => {
              ...
              return (
                <div key={block.title}>
                  ...
                  <p className="text-base leading-relaxed mb-6 max-w-2xl" style={{ color: 'var(--color-ink-muted)' }}>
                    {block.description}
                  </p>
