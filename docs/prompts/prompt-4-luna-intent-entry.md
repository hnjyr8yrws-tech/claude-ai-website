...
- **Version:** r2
- **Date:** 2026-07-04
- **Status:** Reviewed by CR + CD
- **Notes:** Applied Luna Entry Screen spec updates (§4.1–§4.6). Nine-role set adopted. Composer is permanently live from first paint.

Known open item (outside §4 scope, awaiting CR): Section A item 1 still reads "six role buttons / role-declaration step" — inconsistent with the adopted nine-role CONTEXT and A.0.
...
entry flow (nine optional role chips: Teacher / SLT / SEND Lead / DSL / Parent /
Student / IT Manager / Finance / Admin) and a permanently live chat composer on
the entry screen. This task adds a second optional question to the flow to capture
use-case intent, routing users to one of two structurally different response modes.
...
0. Entry state: the widget's opening screen presents the canonical welcome
   message, an optional role chip row, three exemplar questions, and a permanently
   visible chat composer. No question in this flow ever gates the composer. Role
   selection is optional context, not a step.
1. Preserves the existing role-declaration step (six role buttons)
2. After role is declared, Luna asks: "One quick question, then I'll get straight
   to it — are you looking for something new, or checking a tool your school
   already uses?" This question is optional: the chat input stays live, and if the
   user simply types instead of choosing a button, Luna does not force a button
   choice — it infers intent per item 4 and proceeds.
...
4. Intent inference: if the user types rather than clicking intent buttons, Luna
   ...
   4a. Query contains a named tool → default to AUDIT mode (ask: "I can see you're
       asking about [Tool] — would you like its current Promptly Score and an
       option to save the receipt?")
   4b. Query describes a use case → default to DISCOVERY mode
   4c. If the user types first and no role has been declared: infer the role if it
       is stated or unambiguous in the message. Otherwise, before the first
       recommendation, ask once: "Before I answer properly — what's your role? It
       changes what I'd point you at." Present the role chips inline with a
       "Skip — just answer" option. If skipped, proceed with a general answer
       prefixed: "I'll keep this general. Tell me your role any time and I'll
       tailor it." This is the single question used; intent has been inferred, so
       the Two-Question Rule holds.
...
B0. Entry screen: composer docked and live from first paint; role chips optional;
    three Donna-passed exemplar questions; fail-closed offline state.
...
C. Four sample conversation flows (prose, not code):
Write four complete short conversations in Luna's voice demonstrating:
...
4. A user who types first with no role declared ("we're thinking about Diffit for
   Year 8") — demonstrating intent inference to AUDIT mode, the single role
   check-in, the skip path, and the general-answer caveat.

All four must demonstrate: British Voice, provenance stamp on Pillar Card, Rule 4b
...
3. Four sample conversation flows
