...
export default function EmailCapture({ ctaLabel, roles = [], source = "website", successText = "You're on the list — we'll be in touch.", onSuccess }: Props) {
  const { subscribe, status, error, reset } = useBrevo();
  const [email, setEmail] = useState("");

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const ok = await subscribe({ email, roles, source });
